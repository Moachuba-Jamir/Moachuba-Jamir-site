...
function responsive() {
  ...
  if (window.innerWidth <= 500) {
    ...
    script.type = "module";
    script.src =
      "https://unpkg.com/@splinetool/viewer@1.0.53/build/spline-viewer.js";
...
document.getElementById("myForm").addEventListener("submit", function (event) {
  ...
  var isEmptyField = Array.from(formData.values()).some(
    (value) => value.trim() === ""
  );
  if (isEmptyField) {
  ...
  fetch(form.action, {
    ...
    .catch((error) => {
      ...
      alert(
        "An error occurred. Check your internet connection Please try again later."
      );
    });
});
